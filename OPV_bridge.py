#Python code for running posture detection and correction for the exercise of Glute Bridges
...
input_source = "bridge.mp4"	#change name for required video to be analyzed
...
c1=0
x=[]
for i in range(0,len(a)):   #this loop is responsible for marking the highest points of the hip angle in the video to be checked
    c1+=1
    if(h1[i]<=h1_mean):
        if(c1==1):
            x1=i
    if(h1[i]>h1_mean):
        if(c1>10):
            x2=i
            x.append(round((x1+x2)/2))
            #print(x1,x2)
        c1=0 
print("Frames to check the bridges=",x)
for i in x:     #once the required highest frames and determined the angle at these points is to be checked and provide correction
    [a12_m,a23_m,a34_m,a15_m,a56_m,a67_m]=a[i]
    print("\n",a[i])
        
    
    if((a12_m<=10 or a15_m<=10) and (a23_m<=10 or a56_m<=10)):  #the angle of the hip is checked for being in the required range to be classified as a good bridge
        print("Good Bridge")
        if (a12_m<=a15_m):  #once determined that it is a good bridge, other factors of the posture are checked like the back angle
            print("Just",a12_m," degrees for a perfectly straight back")
        else:
            print("Just",a15_m," degrees for a perfectly straight back")
        if (a23_m<=a56_m):
        ...
        print("Bad Bridge")     #if determined as a bad bridge, the factors contibuting to that are mentioned and required corrections are provided
        ...
        if (a23_m>10 and a56_m>10):
            print("Maintain the hip in the line of a straight back")
        if (a23_m<=a56_m):
            print("Hip bend=",a23_m)
        else:
            print("Hip bend=",a56_m)
